...
object POSAdvertisePreference {
    ...
    fun setInternalZipFileExtracted(context: Context?, versionName: String, value: Boolean) {
        BasePrefUtil.setBoolean(context, INTERNAL_ZIP_FILE + versionName, value)
    }

    fun isInternalZipFileExtracted(context: Context?, versionName: String): Boolean {
        return BasePrefUtil.getBoolean(context, INTERNAL_ZIP_FILE + versionName, false)
    }
    ...
    fun getAdvertiseVersionCode(context: Context?): Int {
        var defaultValue = 0
        try {
            val configModel = GsonParser.fromJson(getConfigMainJson(context), ConfigModel::class.java)
