...
object POSScreenSaver {
    ...
    fun setUserInteractionCallbackOnDialog(window: Window?, activity: Activity?) {
        timeOutHandler?.setUserInteractionCallbackOnDialog(window, activity)
    }
    ...
    fun getSleepModeTime(): Long {
        return POSAdvertiseConstants.DefaultSleepModeTime
    }
    ...
    fun applyFreeze(context: Context, isFreeze: Boolean) {
        if(isFreeze) {
